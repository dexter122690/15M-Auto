...
(function () {
  ...
  async function acceptSession(session) { currentSession = session; if (!session) { syncReady = false; showSignIn('Sign in to use the shared records.'); return; } const approval = await client.from('user_approvals').select('status').eq('user_id', session.user.id).maybeSingle(); if (!isOwner() && !assignedBranch() && (approval.error || !approval.data || approval.data.status !== 'approved')) { showPending(approval.data); return; } const openingBranch = await chooseOpeningBranch(); const originalSave = typeof save === 'function' ? save : null; if (originalSave && !window.__15mCloudSaveWrapped) { window.__15mCloudSaveWrapped = true; save = function () { originalSave(); saveBranchStore(); scheduleUpload(); }; } await loadCloud(openingBranch); }
