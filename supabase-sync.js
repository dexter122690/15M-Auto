...
(function () {
  ...
  function showSignIn(message) {
    ...
    if (!modal) {
      ...
      const getValues = function () { return { branch: branchPicker.value, email: emailField.value.trim(), password: document.getElementById('cloudPassword').value }; };
      const note = function (text) { document.getElementById('cloudMessage').textContent = text; };
      branchPicker.onchange = function () {
        const email = this.value;
        const isBranch = !!BRANCH_ACCESS[email];
        emailField.value = isBranch ? email : '';
        emailField.readOnly = isBranch || !email;
      ...
      const signIn = async function () {
        ...
        if (!email || !v.password) return note('Enter the password for this branch.');
        if (v.branch !== 'owner' && email !== v.branch) return note('Use the email assigned to the selected branch.');
        const branch = BRANCH_ACCESS[email];
        if (branch && v.password !== branch.password) return note('Incorrect branch password.');
        note('Signing in...');
  ...
  function clearStoTomasLocalCopy() {
    try {
      const stored = legacyBranchStore(JSON.parse(localStorage.getItem(BRANCH_STORE_KEY) || 'null'));
      if (!stored.branches.some(function (branch) { return branch.id === '15m-sto-tomas'; })) stored.branches.push({ id: '15m-sto-tomas', name: '15M Sto. Tomas' });
      stored.branchData['15m-sto-tomas'] = emptyBranchData();
      stored.selectedBranchId = '15m-sto-tomas';
      localStorage.setItem(BRANCH_STORE_KEY, JSON.stringify(stored));
      localStorage.setItem(LEGACY_DATA_KEY, JSON.stringify(emptyBranchData()));
      if (typeof data !== 'undefined') data = emptyBranchData();
    } catch (_) {}
  }
  function repairDisplayText() {
    const replacements = { 'â‚±': '₱', 'â€”': '—', 'â€“': '–', 'â€™': "'", 'â€œ': '"', 'â€': '"', 'Â ': ' ' };
    const fix = function () {
      const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
      let node;
      while ((node = walker.nextNode())) {
        let text = node.nodeValue, repaired = text;
        Object.keys(replacements).forEach(function (bad) { repaired = repaired.split(bad).join(replacements[bad]); });
  ...
  function activateBranch(id, redraw) {
    ...
    data = id === 'all' ? mergedBranchData() : (branchStore.branchData[id] && Object.keys(branchStore.branchData[id]).length ? branchStore.branchData[id] : (branchStore.branchData[id] = emptyBranchData()));
    saveBranchStore();
  ...
  async function loadCloud(openingBranch) { status('Loading secure records...'); const result = await client.from(TABLE).select('payload').eq('id', ROW_ID).maybeSingle(); if (result.error) { status('Cloud sync needs attention'); console.error('15M cloud load:', result.error); return; } branchStore = legacyBranchStore(result.data && result.data.payload); const refreshKey = '15m-sto-tomas-zero-reset-v2-' + openingBranch; const needsCleanRefresh = sessionStorage.getItem(refreshKey) !== 'yes'; const removedCopies = removeCopiedBranchRecords(); if (!openingBranch || !activateBranch(openingBranch, false)) { status('Branch access is not available for this account.'); return; } if (typeof render === 'function') { try { render(); } catch (error) { console.warn('Cloud records loaded, but this screen needs a display refresh:', error); } } renderBranchControl(); syncReady = true; if (!result.data || !result.data.payload || !result.data.payload.__15mMultiBranch || removedCopies) await upload(); if (needsCleanRefresh) { sessionStorage.setItem(refreshKey, 'yes'); location.reload(); return; } status(isOwner() ? 'Cloud synced — tap for account approvals' : 'Cloud synced'); }
  async function acceptSession(session) { currentSession = session; if (!session) { syncReady = false; showSignIn('Sign in to use the shared records.'); return; } const directBranch = assignedBranch(); const approval = directBranch ? { data: { status: 'approved' } } : await client.from('user_approvals').select('status').eq('user_id', session.user.id).maybeSingle(); if (!isOwner() && !directBranch && (approval.error || !approval.data || approval.data.status !== 'approved')) { showPending(approval.data); return; } if (directBranch) removeModal('cloudSignIn'); const openingBranch = directBranch ? directBranch.id : (function () { try { const saved = JSON.parse(localStorage.getItem(BRANCH_STORE_KEY) || '{}'); return saved && saved.selectedBranchId && saved.selectedBranchId !== 'all' ? saved.selectedBranchId : 'sta-rosa'; } catch (_) { return 'sta-rosa'; } }()); removeModal('branchGate'); const originalSave = typeof save === 'function' ? save : null; if (originalSave && !window.__15mCloudSaveWrapped) { window.__15mCloudSaveWrapped = true; save = function () { originalSave(); saveBranchStore(); scheduleUpload(); }; } await loadCloud(openingBranch); }
  // The Dashboard is available to every signed-in branch without a second passcode.
  function protectDashboard() {}

  async function begin() { addStyles(); applyBrandLogo(); repairDisplayText(); protectDashboard(); const script = document.createElement('script'); script.src = 'https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2'; script.onload = async function () { client = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY); const session = (await client.auth.getSession()).data.session; await acceptSession(session); client.auth.onAuthStateChange(function (_event, nextSession) { setTimeout(function () { acceptSession(nextSession); }, 0); }); }; script.onerror = function () { status('Cloud sync library could not load'); }; document.head.appendChild(script); }
  window.__15mCloudReady = false;
  const cloudReadyWatch = setInterval(function () {
    const cloudNotice = document.getElementById('cloudStatus');
    if (cloudNotice && /^Cloud synced/.test(cloudNotice.textContent || '')) {
      window.__15mCloudReady = true;
